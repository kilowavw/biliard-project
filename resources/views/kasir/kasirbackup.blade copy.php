...
<div class="container mx-auto p-4">
    ...
    <div id="meja-container" class="grid grid-cols-1 sm:grid-cols-2 [@media(min-width:768px)_and_(max-width:870px)]:grid-cols-2
        ...
        <div id="meja-card-{{ $meja->id }}"
            ...
            <div id="detail-{{ $meja->id }}" class="hidden mt-3 space-y-2" onclick="event.stopPropagation();">
                ...
                <button id="btn-pesan-{{ $meja->id }}" onclick="event.stopPropagation(); openModal({{ $meja->id }})"
...
{{-- Modal Pemesanan --}}
<div id="pesanModal" style="display:none"
        ...
        <form id="formPesan" method="POST">
            ...
            <div class="mb-3">
                ...
                <div class="flex">
                    ...
                    <button type="button" id="btn-validate-member"
            ...
            <div class="mb-3">
                <label for="nama_penyewa" class="block text-sm font-medium text-gray-700">Nama Penyewa <span
            ...
            <div class="mb-3">
                <label for="paket_id_select" class="block text-sm font-medium text-gray-700">Pilih Paket
                    (Opsional)</label>
            ...
            <div id="non_paket_options" class="space-y-3">
                ...
                <div class="mb-3" id="durasi_jam_wrapper">
                    <label for="durasi_jam" class="block text-sm font-medium text-gray-700">Durasi (Jam, contoh: 1.5
                        untuk 1 jam 30 menit)</label>
...
{{-- Modal Tambah Durasi --}}
<div id="addDurationModal" style="display:none"
    class="fixed inset-0 bg-black bg-opacity-75 flex items-center justify-center z-50">
    <div class="bg-white p-6 rounded-lg shadow-xl w-full max-w-md text-black">
        ...
        <form id="formAddDuration">
            ...
            <div class="mb-3">
                <label for="additional_durasi_jam" class="block text-sm font-medium text-gray-700">Durasi Tambahan
                    (Jam)</label>
                <input type="number" name="additional_durasi_jam" id="additional_durasi_jam" required min="0.01"
                    step="0.01"
                    class="mt-1 block w-full border-gray-300 rounded-md shadow-sm focus:ring-blue-500 focus:border-blue-500 sm:text-sm p-2">
            </div>
            <div class="flex justify-end mt-4 space-x-2">
                <button type="button" onclick="closeAddDurationModal()"
                    class="px-4 py-2 border border-gray-300 rounded-md shadow-sm text-sm font-medium text-gray-700 bg-white hover:bg-gray-50 focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-blue-500">Batal</button>
                <button type="submit"
                    class="px-4 py-2 bg-blue-600 border border-transparent rounded-md shadow-sm text-sm font-medium text-white hover:bg-blue-700 focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-blue-500">Tambah
                    Durasi</button>
            </div>
        </form>
    </div>
...
<div id="addServiceModal" style="display:none"
    ...
    <div class="bg-white p-6 rounded-lg shadow-xl w-full max-w-lg text-black">
        ...
        <form id="formAddService">
            ...
            <p class="text-lg font-bold text-gray-800">Total Tambahan Service: <strong id="current_service_add_total"
                    class="text-neutral-600">Rp 0</strong></p>
...
<div id="paymentModal" style="display:none"
    ...
    <div class="bg-white p-6 rounded-lg shadow-xl w-full max-w-lg max-h-[90vh] overflow-y-auto text-black">
        ...
        <form id="formPembayaran">
            ...
            <p id="payment_mode_sepuasnya_info" style="display: none;" class="text-sm text-gray-500 italic">(Mode Main
                Sepuasnya)</p>
            ...
            <p class="text-lg font-bold mb-1">Diskon Member: <strong id="payment_diskon_member"
                    class="text-red-500"></strong></p>
            <p class="text-lg font-bold mb-1">Diskon Kupon: <strong id="payment_diskon_kupon"
                    class="text-red-500"></strong></p>
            <p class="text-xl font-bold mt-2">Total Pembayaran: <strong id="payment_total_final"
            ...
            <div class="flex justify-end mt-4 space-x-2">
                <button type="button" onclick="closePaymentModal()"
                    class="px-4 py-2 border border-gray-300 rounded-md shadow-sm text-sm font-medium text-gray-700 bg-white hover:bg-gray-50 focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-blue-500">Batal</button>
                <button type="submit"
                    class="px-4 py-2 bg-blue-600 border border-transparent rounded-md shadow-sm text-sm font-medium text-white hover:bg-blue-700 focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-blue-500">Bayar
                    Sekarang</button>
                <button type="button" id="btn-pay-qris" onclick="handleQrisPayment()"
                    class="px-4 py-2 bg-neutral-600 text-white rounded-md hover:bg-neutral-700 focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-neutral-500">Bayar
                    dengan QRIS</button>
            </div>
        </form>
    </div>
...
const toggleModal = (modalId, show) => {
    getEl(modalId).style.display = show ? 'flex' : 'none';
};
const fmtTime = (dtStr) => {
    if (!dtStr) return '--:--';
...
const fmtRp = (amt) => new Intl.NumberFormat('id-ID', {
    style: 'currency',
    currency: 'IDR',
...
const openAddDurationModal = (penyewaanId) => {
    const p = currentActiveRentals[penyewaanId];
    if (!p || p.is_sepuasnya) {
        Swal.fire('Info', 'Tidak bisa menambah durasi untuk penyewaan ini.', 'info');
        return;
    }
    getEl('add_duration_penyewaan_id').value = penyewaanId;
    getEl('add_duration_meja_nama').innerText = p.meja_nama;
    getEl('add_duration_nama_penyewa').innerText = p.nama_penyewa;
    getEl('add_duration_current_durasi').innerText = fmtDur(p.durasi_jam);
    getEl('formAddDuration').reset();
    toggleModal('addDurationModal', true);
};
const closeAddDurationModal = () => toggleModal('addDurationModal', false);
const openAddServiceModal = async (penyewaanId) => {
    const p = currentActiveRentals[penyewaanId];
    if (!p) {
        Swal.fire('Error', 'Penyewaan tidak ditemukan.', 'error');
        return;
    }
    getEl('add_service_penyewaan_id').value = penyewaanId;
    getEl('add_service_meja_nama').innerText = p.meja_nama;
    getEl('add_service_nama_penyewa').innerText = p.nama_penyewa;
    getEl('add_service_current_total').innerText = fmtRp(p.total_service);
    getEl('current_service_add_total').innerText = fmtRp(0);
...
const openPaymentModal = (penyewaanId) => {
    ...
    getEl('payment_penyewaan_id').value = penyewaanId;
    getEl('kode_kupon').value = '';
...
const startCountdown = (el, waktuSelesaiStr, penyewaanData) => {
    ...
    const pId = penyewaanData.id;
    if (countdownIntervals[pId]) clearInterval(countdownIntervals[pId]);
    const interval = setInterval(() => {
        const now = getCalibratedNow();
        const dist = waktuSelesai.getTime() - now.getTime();
        if (dist <= 0) {
            clearInterval(interval);
            el.innerText = 'Waktu habis!';
            el.classList.add('text-red-700', 'font-bold');
            handleTimeUpUI(penyewaanData);
        } else {
            const h = String(Math.floor((dist / (1000 * 60 * 60)) % 24)).padStart(2, '0');
            const m = String(Math.floor((dist / (1000 * 60)) % 60)).padStart(2, '0');
            const s = String(Math.floor((dist / 1000) % 60)).padStart(2, '0');
            el.innerText = `${h}:${m}:${s}`;
        }
    }, 1000);
    countdownIntervals[pId] = interval;
};
...
const startRunningTimer = (el, waktuMulaiStr, penyewaanData) => {
    ...
    const interval = setInterval(() => {
        const now = getCalibratedNow();
        const elapsed = now.getTime() - waktuMulai.getTime();
        const h = String(Math.floor((elapsed / (1000 * 60 * 60)) % 24)).padStart(2, '0');
        const m = String(Math.floor((elapsed / (1000 * 60)) % 60)).padStart(2, '0');
...
const handleTimeUpUI = (penyewaanData) => {
    ...
    if (mejaCardEl) {
        ...
        'bg-blue-100'); // Hapus semua background sebelumnya
...
const updateMejaStatusOnServer = async (mejaId, newStatus) => {
    try {
        ...
        if (!res.ok) {
            ...
            Swal.fire('Error', 'Gagal memperbarui status meja di server: ' + (data.message ||
...
const toggleServiceDropdown = (penyewaanId) => {
    ...
    if (serviceListEl.style.display === 'block') {
        serviceListEl.style.display = 'none';
        iconEl.classList.replace('fa-chevron-up', 'fa-chevron-down');
    } else {
        serviceListEl.style.display = 'block';
        ...
        if (result.isConfirmed) {
            try {
                const res = await fetch(
                    `{{ url('/kasir/penyewaan/') }}/${penyewaanId}/remove-service`, {
                        method: 'DELETE',
                        headers: {
                            'Content-Type': 'application/json',
                            'X-CSRF-TOKEN': '{{ csrf_token() }}'
                ...
                Swal.fire('Error!',
                    'Terjadi kesalahan jaringan atau server saat menghapus service.', 'error');
            }
        }
    });
};
...
const fetchAndRenderMejas = async () => {
    try {
        const res = await fetch('{{ route('
            kasir.api.penyewaanAktif ') }}');
        const activeRentals = await res.json();
        currentActiveRentals = {};
        activeRentals.forEach(p => currentActiveRentals[p.id] = p);
        ...
        document.querySelectorAll('[id^="meja-card-"]').forEach(card => {
            ...
            if (penyewaanForThisMeja) {
                card.classList.remove('bg-neutral-600');
                ...
                const isTimeUp = penyewaanForThisMeja.waktu_selesai && (new Date(penyewaanForThisMeja
                    .waktu_selesai)).getTime() - getCalibratedNow().getTime() <= 0;
                const isSepuasnya = penyewaanForThisMeja.is_sepuasnya;

                let timerDisplay =
                `â³ <span id="countdown-${penyewaanForThisMeja.id}">--:--:--</span>`;
                if (isSepuasnya) timerDisplay =
                    `â³ <span id="running-timer-${penyewaanForThisMeja.id}">00:00:00</span> (Main Sepuasnya)`;
                else if (isTimeUp) timerDisplay = `â³ Waktu habis!`;
                ...
                if (penyewaanForThisMeja.service_detail && penyewaanForThisMeja.service_detail.length >
                    0) {
                ...
                if (penyewaanForThisMeja.member_id) {
                    memberInfoHtml =
                        `<p class="text-sm font-medium text-blue-700">Member: ${penyewaanForThisMeja.member_nama} (${penyewaanForThisMeja.member_kode})</p>`;
                }
                ...
                if (penyewaanDivEl) {
                    penyewaanDivEl.innerHTML = `
                        ...
                        `;
                    const timerEl = isSepuasnya ? getEl(`running-timer-${penyewaanForThisMeja.id}`) :
                        getEl(`countdown-${penyewaanForThisMeja.id}`);
                    if (timerEl) {
                        if (isSepuasnya) startRunningTimer(timerEl, penyewaanForThisMeja.waktu_mulai,
                            penyewaanForThisMeja);
                        else if (penyewaanForThisMeja.status === 'berlangsung' && !isTimeUp)
                            startCountdown(timerEl, penyewaanForThisMeja.waktu_selesai,
                                penyewaanForThisMeja);
                        else if (isTimeUp) {
                            timerEl.innerText = 'Waktu habis!';
                            timerEl.classList.add('text-red-700', 'font-bold');
                        }
                    }
                }
            } else {
                ...
                if (!pesanBtnEl) {
                    ...
                    pesanBtnEl.className =
                        'mt-2 px-4 py-2 bg-blue-500 text-white rounded hover:bg-blue-600 focus:outline-none focus:ring-2 focus:ring-blue-500 focus:ring-opacity-50';
...
const fetchPaymentDetails = async (penyewaanId) => {
    const p = currentActiveRentals[penyewaanId];
    if (!p) {
        Swal.fire('Error', 'Detail penyewaan tidak ditemukan.', 'error');
        closePaymentModal();
        return;
    }
    ...
    if (p.is_sepuasnya) {
        durasiUntukPerhitungan = (waktuSelesaiAktual.getTime() - new Date(p.waktu_mulai).getTime()) / (1000 *
            60 * 60);
    }
    ...
    const updateTotalDisplay = (diskonPersenKupon = 0) => {
        ...
        const diskonKuponAmount = (totalBeforeAllDiscounts * diskonPersenKupon) /
        100; // Ini adalah diskon kupon yang 'seharusnya' tanpa batasan 100%
    ...
    getEl('kode_kupon').oninput = debounce(async function() {
        ...
        if (kuponCode) {
            try {
                const res = await fetch(
                    `{{ route('api.kupon.validate') }}?code=${encodeURIComponent(kuponCode)}`);
                if (res.ok) {
                    const data = await res.json();
                    diskonPersen = parseFloat(data.diskon_persen) || 0;
                    Swal.fire('Berhasil!',
                        `Kupon "${kuponCode}" berhasil diterapkan! Diskon ${diskonPersen}%`,
                        'success');
                } else {
                    const errData = await res.json();
                    Swal.fire('Error', 'Kupon tidak valid: ' + (errData.message ||
                        'Kode kupon tidak ditemukan atau kadaluarsa.'), 'error');
                }
            } catch (e) {
                Swal.fire('Error', 'Terjadi kesalahan jaringan atau server saat memvalidasi kupon.',
                    'error');
...
const fetchAndRenderServicesForAdd = async () => {
    try {
        ...
        const serviceListContainer = getEl('service_list_container');
        ...
        allAvailableServices.forEach(s => {
            ...
            row.innerHTML =
                `<span class="font-medium text-gray-800">${s.nama} (${fmtRp(s.harga)}) <span class="text-gray-500 text-xs">(Stok: ${s.stok})</span></span>
            ...
            input.addEventListener('input', () => {
                ...
                serviceListContainer.querySelectorAll('.service-qty').forEach(qtyInput => {
                    ...
                    if (qty > maxStock) {
                        ...
                        Swal.fire('Info',
                            `Jumlah ${qtyInput.closest('div').querySelector('span').firstChild.textContent.trim()} melebihi stok yang tersedia (${maxStock}).`,
                            'info');
        ...
        Swal.fire('Error', 'Gagal memuat daftar service.', 'error');
...
getEl('kode_member').addEventListener('input', debounce(async function() {
    ...
    if (kodeMember.length > 0) {
        try {
            const res = await fetch(
                `{{ route('api.member.validate') }}?kode_member=${encodeURIComponent(kodeMember)}`, {
                    headers: {
            ...
            if (res.ok && data.valid) {
                // Jika member valid
                memberData = data;
                memberInfoPreview.innerText =
                    `Member: ${data.nama_member} (Diskon: ${data.diskon_persen}%)`;
...
getEl('paket_id_select').addEventListener('change', function() {
    ...
    if (selectedPaketId) {
        ...
        if (selectedPaket) {
            ...
            if (isiPaket.services && isiPaket.services.length > 0) desc.push(
                `Service: ${isiPaket.services.length} item`);
...
getEl('formAddDuration').addEventListener('submit', async (e) => {
    e.preventDefault();
    const pId = getEl('add_duration_penyewaan_id').value;
    const addDur = getEl('additional_durasi_jam').value;
    const submitBtn = e.target.querySelector('button[type="submit"]');
    submitBtn.disabled = true;
    submitBtn.innerHTML = '<i class="fa-solid fa-spinner fa-spin"></i> Menambah...';
    try {
        const res = await fetch(`{{ url('/kasir/penyewaan/') }}/${pId}/add-duration`, {
            method: 'POST',
            headers: {
                'Content-Type': 'application/json',
                'X-CSRF-TOKEN': '{{ csrf_token() }}'
        ...
        Swal.fire('Error!', 'Terjadi kesalahan jaringan atau server saat menambah durasi.', 'error');
        console.error('Error adding duration:', error);
    } finally {
        submitBtn.disabled = false;
        submitBtn.innerHTML = 'Tambah Durasi';
    }
});

getEl('formAddService').addEventListener('submit', async (e) => {
    e.preventDefault();
    const pId = getEl('add_service_penyewaan_id').value;
    const selSrv = [];
    ...
    Array.from(getEl('service_list_container').querySelectorAll('.service-qty')).forEach(input => {
        ...
        const serviceName = input.closest('div').querySelector('span').firstChild.textContent
    .trim();

        if (qty > 0) {
            if (qty > maxStock) {
                Swal.fire('Info',
                    `Jumlah ${serviceName} (${qty}) melebihi stok yang tersedia (${maxStock}). Mohon koreksi.`,
                    'info');
                stockExceeded = true;
                return;
            }
    ...
    if (selSrv.length === 0) {
        Swal.fire('Info', 'Pilih setidaknya satu service untuk ditambahkan.', 'info');
        return;
    }
    ...
    submitBtn.innerHTML = '<i class="fa-solid fa-spinner fa-spin"></i> Menambah...';
    try {
        const res = await fetch(`{{ url('/kasir/penyewaan/') }}/${pId}/add-service`, {
            method: 'POST',
            headers: {
                'Content-Type': 'application/json',
                'X-CSRF-TOKEN': '{{ csrf_token() }}'
        ...
        Swal.fire('Error!', 'Terjadi kesalahan jaringan atau server saat menambah service.', 'error');
        console.error('Error adding service:', error);
    } finally {
        submitBtn.disabled = false;
        submitBtn.innerHTML = 'Tambah Service';
    }
});

getEl('formPembayaran').addEventListener('submit', async (e) => {
    e.preventDefault();
    const pId = getEl('payment_penyewaan_id').value;
    const kupon = getEl('kode_kupon').value;
    const submitBtn = e.target.querySelector('button[type="submit"]');
    submitBtn.disabled = true;
    submitBtn.innerHTML = '<i class="fa-solid fa-spinner fa-spin"></i> Memproses...';
    try {
        const res = await fetch(`{{ url('/kasir/penyewaan/') }}/${pId}/bayar`, {
            method: 'POST',
            headers: {
                'Content-Type': 'application/json',
                'X-CSRF-TOKEN': '{{ csrf_token() }}'
        ...
        const data = await res.json();
        if (res.ok) {
            Swal.fire('Berhasil!', data.message + ' Total bayar: ' + fmtRp(data.total_bayar), 'success');
            closePaymentModal();
            if (countdownIntervals[pId]) {
        ...
        Swal.fire('Error!', 'Terjadi kesalahan jaringan atau server saat memproses pembayaran.', 'error');
    ...
    try {
        ...
        if (res.ok) {
            Swal.fire({
                ...
                text: data.message + ' Total bayar: ' + fmtRp(data.total_bayar),
                confirmButtonText: 'OK'
            });
            closePaymentModal();
            ...
            Swal.fire({
                ...
                text: data.message || 'Terjadi kesalahan saat memproses pembayaran QRIS.',
                confirmButtonText: 'OK'
            });
        ...
        Swal.fire({
            ...
            text: 'Terjadi kesalahan jaringan atau server saat memproses pembayaran QRIS.',
            confirmButtonText: 'OK'
        });
...
const confirmDeletePenyewaan = async (penyewaanId, mejaNama) => {
    if (userRole !== 'admin') {
        Swal.fire({
            icon: 'error',
            title: 'Akses Ditolak!',
            text: 'Penghapusan meja hanya bisa dilakukan oleh Supervisor.',
            confirmButtonText: 'OK'
        });
        return;
    }
...
function toggleDetail(mejaId) {
    ...
    if (activeCardId === mejaId) {
        // Kalau klik card yg sama â†’ toggle hide
